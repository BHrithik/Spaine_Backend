---
page_type: sample
description: Connecting ESPRESSIF ESP32 to Azure IoT Central using the Azure SDK for C Arduino library for getting information from an accelerometer
languages:
- c
products:
- azure-iot
...
In this Project I usee the Azure SDK for C to connect the ESPRESSIF ESP32 to Azure IoT Central. This is a modified tutorial from Azure to get data from an MPU connected to the ESP32 to Azure IOT central.

The series introduces device developers to the Azure SDK for C, and shows how to connect several device evaluation kits to Azure IoT.

This can be successfully run on our local machine and on the ESP32 only after we complete the following tasks:
...
There are several ways to connect devices to Azure IoT. IoT Central is an IoT application platform that reduces the cost and complexity of creating and managing IoT solutions.

To create a new application we follow the following steps:
...
If we follow all the steps provided above we see that the telementary data is just a simulation and does not change.
to get the data we need from the ESP we need to do the following steps.
1. Go to the Azure_IoT_Central_ESP32.ino file and include the code for the ESP to read data from the PINs. In this repositry you will find code to read from an MPU this can be replaced with code to read from any kind of sensor.
1. Output the data obtained from the sensor in loop() to a particular pin which is free using analog write command.
1. Go to the file Azure_IoT_PnP_Template.cpp file and read the relevant data in the generate telementary data function by using analogRead(*PIN_NUMBER*).
1. Now we should be able to get the data we require from the ESP to our cpp file and send it to Azure IOT central by replacing the constants with the actual analogRead function.

Note:- As we have limited resources we might run into issues such as out of memory. We need to minimize the number of libraries to save the maximum space and declare local variables where ever necessary.

We can configure the dashboard with the data we obtain and structure it using the UI in azure IOT central.
