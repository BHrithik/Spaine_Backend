...
/* --- Defines --- */
#define AZURE_PNP_MODEL_ID "dtmi:pillowtalkTeam12HelloWorld:PillowCore_39m;1"
...
#define SAMPLE_MODEL_PROPERTY_VALUE "ESP-WROOM-32"
...
static void get_accel_value(int32_t* accelerationX,
    int32_t* accelerationY,
    int32_t* accelerationZ) {
  *accelerationX = analogRead(X_PIN);
  *accelerationY = analogRead(Y_PIN);
  *accelerationZ = analogRead(Z_PIN);
}
    ...
    size_t* payload_buffer_length)
{
  ...
  get_accel_value(&accelerationX, &accelerationY, &accelerationZ);
  ...
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding ACCELEROMETERX value property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, accelerationX);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding ACCELEROMETERX value property value to telemetry payload.");

  rc = az_json_writer_append_property_name(
      &jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_ACCELEROMETERY));
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding ACCELEROMETERY value property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, accelerationY);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding ACCELEROMETERY value property value to telemetry payload.");

  rc = az_json_writer_append_property_name(
      &jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_ACCELEROMETERZ));
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding ACCELEROMETERZ value property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, accelerationZ);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding ACCELEROMETERZ value property value to telemetry payload.");
